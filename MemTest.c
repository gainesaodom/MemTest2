/*
General Purpose SPI API for Memory chips with 24-bit address space
...
#include <stdint.h>
#include <stdbool.h>
#include "SerialSPI16MB.c"


uint8_t checkerboard_compute_byte_value_at_address(uint32_t address, bool checkerboard_starts_with_a_zero_bit) {
...
uint8_t read_bit(uint32_t address, uint8_t bitpos) {
	...
	uint8_t byte = spi_mem_read_byte(address);
...
void write_bit(uint32_t address, uint8_t bitpos, uint8_t value) {


	uint8_t byte = spi_mem_read_byte(address);
	byte = set_bit_in_byte(byte, bitpos, value);
	spi_mem_write_byte(address, byte);

}

void reset_memory_to_zeroes() {
	uint16_t address_pointer;
	for (address_pointer = 0; address_pointer <= SPI_MEM_MAX_ADDRESS; address_pointer++) {
		spi_mem_write_byte(address_pointer, 0x00);
	}

}

void print_value_at_address(uint32_t address) {
	printf("Address: %04" PRIx32 " \tValue: %02" PRIx8 "\n", address, spi_mem_read_byte(address));
}
...
uint16_t march_A_test() {
	...
	for (address_idx = 0; address_idx <= SPI_MEM_MAX_ADDRESS; address_idx++) {
		...
		for (i = 0; i < 8; i++) {
			...
			if (expected_value != actual_value) {	// read 0
				number_of_faults++;
				printf("M1 Fault -- %02" PRIx8 " at %04" PRIx32 " when it should be %02" PRIx16 "\n", actual_value, address_idx, expected_value);
				//exit(EXIT_FAILURE);
			}	
	...
	for (address_idx = SPI_MEM_MAX_ADDRESS; address_idx >= 0; address_idx--) {
		...
		for (i = 7; i >= 0; i--) {
			...
			if (actual_value != expected_value) {
				number_of_faults++;
		 		printf("M3 Fault -- %02" PRIx8 " at %04" PRIx32 " when it should be %02" PRIx16 "\n", actual_value, address_idx, expected_value);
				//exit(EXIT_FAILURE);
			}
	...
	for (address_idx = SPI_MEM_MAX_ADDRESS; address_idx >= 0; address_idx--) {
		...
		for (i = 7; i >= 0; i--) {
			...
			uint8_t actual_value = read_bit(address_idx, i);//((spi_mem_read_byte(address_idx) >> i) & 1);

			//printf("Read value %04" PRIx16 "\n", actual_value);

			if (expected_value != actual_value) {	// read 0
				number_of_faults++;
				printf("Fault -- %02" PRIx8 " at %04" PRIx32 " when it should be %02" PRIx16 "\n", actual_value, address_idx, expected_value);
				//exit(EXIT_FAILURE);
			}	
...
/*
	Runs a checkerboard test on the Memory chip using SPI
...
uint16_t checkerboard_test(bool first_bit_is_zero) {
	...
	for (address_idx = 0; address_idx <= SPI_MEM_MAX_ADDRESS; address_idx++) {
		...
		uint8_t actual_value = spi_mem_read_byte(address_idx);	
